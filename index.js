...
const postscollection = database.collection('news');
...
addBtn.addEventListener('click', e => {
  ...
  ID.set({
    ...
    .then(response => alert("Check Explorer!"))
...
async function pay(){
    ...
    contract = new web3.eth.Contract(abi,address)
      ...
      var link=document.getElementById("type").value;

        const accounts = await web3.eth.getAccounts()

        
        contract.methods.set(link).send({from:accounts[0]}) 
